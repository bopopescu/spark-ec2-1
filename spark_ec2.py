...
DEFAULT_SPARK_EC2_GITHUB_REPO = "https://github.com/lingkio/spark-ec2-lingk"
...
def setup_cluster(conn, master_nodes, slave_nodes, opts, deploy_ssh_key):
    ...
    ssh(master, opts, "echo '/root/spark/sbin/start-master.sh' >> /etc/rc.local")
    ssh(master, opts, "echo '/root/ephemeral-hdfs/sbin/start-dfs.sh' >> /etc/rc.local")
    for slave in slave_nodes:
        slave_address = get_dns_name(slave, opts.private_ips)
        active_master = "%s" % (get_dns_name(master_nodes[0], opts.private_ips))
        slave_command = "echo '/root/spark/sbin/start-slave.sh spark://" + str(active_master) + ":7077' >> /etc/rc.local"
        ssh(slave_address, opts, slave_command)
        ssh(slave_address, opts, "echo '/root/ephemeral-hdfs/sbin/start-dfs.sh' >> /etc/rc.local")
...
def deploy_user_files_to_path(root_dir, opts, master_nodes, path):
    print("rsyincing " + root_dir)
    active_master = get_dns_name(master_nodes[0], opts.private_ips)
    command = [
        'rsync', '-rv',
        '-e', stringify_command(ssh_command(opts)),
        "%s" % root_dir,
        "%s@%s:%s" % (opts.user, active_master, path)
    ]
    subprocess.check_call(command)
