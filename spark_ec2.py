...
DEFAULT_SPARK_EC2_GITHUB_REPO = "https://github.com/lingkio/spark-ec2"
...
def parse_args():
    ...
    parser.add_option(
        ...
        help="Apply the tags given in --additional-tags to any EBS volumes " +
             "attached to main and subordinate instances.")
    parser.add_option(
...
#    )
    ...
    print("setting up /etc/rc.local")
    ssh(main, opts, "echo '/root/spark/sbin/start-main.sh' >> /etc/rc.local")
    ssh(main, opts, "echo '/root/ephemeral-hdfs/sbin/start-dfs.sh' >> /etc/rc.local")
    for subordinate in subordinate_nodes:
        subordinate_address = get_dns_name(subordinate, opts.private_ips)
        active_main = "%s" % (get_dns_name(main_nodes[0], opts.private_ips))
        subordinate_command = "echo '/root/spark/sbin/start-subordinate.sh spark://" + str(active_main) + ":7077' >> /etc/rc.local"
        ssh(subordinate_address, opts, subordinate_command)
        ssh(subordinate_address, opts, "echo '/root/ephemeral-hdfs/sbin/start-dfs.sh' >> /etc/rc.local")
...
def deploy_user_files_to_path(root_dir, opts, main_nodes, path):
    print("rsyincing " + root_dir)
    active_main = get_dns_name(main_nodes[0], opts.private_ips)
    command = [
        'rsync', '-rv',
        '-e', stringify_command(ssh_command(opts)),
        "%s" % root_dir,
        "%s@%s:%s" % (opts.user, active_main, path)
    ]
    subprocess.check_call(command)
